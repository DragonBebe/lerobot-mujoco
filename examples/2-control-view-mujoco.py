...
def display_position_info(key_name=None):
    ...
    print(f"🎯 Target: [{', '.join([f'{x:.3f}' for x in target_gpos[:3]])}]")
    print(f"🤖 Actual: [{', '.join([f'{x:.3f}' for x in actual_pos])}]")
    print(f"📐 Orientation (RPY): [{', '.join([f'{x:.3f}' for x in actual_euler])}]")
